...
from bw_processing.io_parquet_helpers import save_arr_to_parquet, load_ndarray_from_parquet
...
def test_save_load_parquet_file(arr_fixture_name, meta_object, meta_type, tmp_path_factory, request):
    ...
    with file as fp:
        save_arr_to_parquet(file=fp, arr=arr, meta_object=meta_object, meta_type=meta_type)
...
def test_save_load_parquet_file_wrong_meta_object(indices_vector, tmp_path_factory):
    ...
    with pytest.raises(NotImplementedError):
        with file as fp:
            save_arr_to_parquet(file=fp, arr=indices_vector, meta_object="wrong", meta_type="indices")

    with pytest.raises(NotImplementedError):
        with file as fp:
            save_arr_to_parquet(file=fp, arr=indices_vector, meta_object="vector", meta_type="indices")
...
def test_save_load_parquet_file_wrong_meta_type(indices_vector, tmp_path_factory):
        ...
        with pytest.raises(NotImplementedError):
            with file as fp:
                save_arr_to_parquet(file=fp, arr=indices_vector, meta_object="vector", meta_type="wrong")
