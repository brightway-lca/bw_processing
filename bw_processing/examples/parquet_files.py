...
if __name__ == '__main__':
    ...
    default_matrix_serialize_format_type = bwp.MatrixSerializeFormat.NUMPY  # bwp.MatrixSerializeFormat.PARQUET
    ...
    if USE_OSFS:
        ...
        dp = bwp.create_datapackage(fs=dp_dir, matrix_serialize_format_type=bwp.MatrixSerializeFormat.NUMPY)
    else:
        # VERSION ZIP
        dp_zip_file = ZipFS(str(dirpath / "datapackage_2.zip"), write=True)
        dp = bwp.create_datapackage(fs=dp_zip_file, matrix_serialize_format_type=bwp.MatrixSerializeFormat.NUMPY)  # bwp.create_datapackage(fs=dp_zip_file, serialize_type=SerializeENum.parquet)
